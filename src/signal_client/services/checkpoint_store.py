...
class IngestCheckpointStore:
    """
    Durable checkpointing for websocket ingest to drop duplicates and resume after restarts.

    We persist a sliding window of recently processed (source, timestamp) pairs. This is
    intentionally coarse but sufficient to dedupe Signal replayed envelopes after reconnects
    or process crashes. The window bounds growth so SQLite-backed bots remain lightweight.
    """
    ...
    async def compact_before(self, cutoff_timestamp: int) -> None:
        ...
        async with self._lock:
            retained = [
                record for record in self._records if record.timestamp >= cutoff_timestamp
            ]
