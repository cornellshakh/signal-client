"""Audit API client parity against the published swagger specification."""
...
from signal_client.adapters.api import (  # noqa: E402
    accounts_client,
    attachments_client,
    contacts_client,
    devices_client,
...
class SwaggerSpecDownloadError(RuntimeError):
    """Raise when downloading the swagger spec fails."""

    def __init__(self, url: str) -> None:
        """Store the failing URL for context."""
...
def get_swagger_spec() -> dict[str, Any]:
    """Load the swagger specification from the upstream hosted location.

    Falls back to a cached file or a user-specified path when offline.
    """
    override_path = os.environ.get(SWAGGER_SPEC_PATH_ENV)
    if override_path:
        path = Path(override_path)
        if not path.is_file():
            message = f"Override swagger spec path '{override_path}' does not exist"
    ...
    try:
        with urlopen(  # noqa: S310  # nosec B310 - scheme validated in _validate_swagger_url
...
def get_client_methods() -> dict[str, set[str]]:
    """Inspect the client modules and return a mapping of client names to methods.

    This function dynamically inspects client modules to extract all public methods.
    """
...
def get_swagger_operations() -> dict[str, set[str]]:
    """Parse the swagger spec and return a mapping of tags (clients) to operationIds.

    This function fetches the OpenAPI (Swagger) specification and extracts
    all defined operation IDs, categorizing them by their associated tags.
    """
...
def audit_api_parity() -> bool:
    """Compare client methods with swagger operations and print discrepancies.

    This function performs a comprehensive audit by comparing the methods
    implemented in the client modules against the operations defined in
    the Signal API's OpenAPI (Swagger) specification. It reports any methods
    present in Swagger but missing in the client, and vice-versa, to ensure
    full API parity.
    """
...
def main() -> None:
    """Run the API parity audit and exit with the appropriate status code."""
