...
from signal_client.infrastructure.schemas.events import (
    BaseEvent,
    BlockEvent,
    CallEvent,
    EnvelopeMetadata,
...
class MessageParser:
    ...
    def recipient_from_raw(self, raw_message_str: str) -> str | None:
    ...
    def _extract_event(
        ...
        if "receiptMessage" in envelope:
            ...
            if isinstance(payload, dict):
                return ReceiptEvent.model_validate(
        ...
        if isinstance(verification, bool):
            return VerificationEvent(
                envelope=metadata, verified=verification
            )
