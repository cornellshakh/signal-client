...
class HealthServer:
    ...
    async def _dlq_status(request: web.Request) -> web.Response:
        ...
        depth = len(entries)
        next_retry_at = min(
            (entry.get("next_retry_at") for entry in entries if isinstance(entry, dict)),
            default=None,
        )
        payload = {
            "enabled": True,
...
def _readiness_payload(application: "Application") -> dict[str, Any]:
    ...
    ready = queue_ready and worker_pool_ready and websocket_configured and api_clients_ready
    queue_depth = (
        application.queue.qsize() if application.queue is not None else None
    )
    return {
        "ready": ready,
        ...
        "ingest_paused_until": intake_snapshot.get("paused_until")
        if intake_snapshot
        else None,
    }
