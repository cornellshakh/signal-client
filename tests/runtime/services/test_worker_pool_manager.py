"""Tests for the worker pool manager."""
...
from signal_client.core.command import Command, command
...
def mock_context_factory():
    """Return a mock context factory."""
    return MagicMock()


@pytest.fixture
def mock_queue():
    """Return a mock asyncio queue."""
    return asyncio.Queue()


@pytest.fixture
def mock_message_parser():
    """Return a mock message parser."""
    return MagicMock()
...
def _build_worker_pool(
    ...
    *,
    ...
    """Helper to build a WorkerPool instance."""
...
def make_raw_message():
    """Return a factory for raw messages."""
...
def _histogram_count() -> float:
    samples = next(iter(MESSAGE_QUEUE_LATENCY.collect())).samples
...
def _command_count(command_name: str, status: str = "success") -> float:
    return COMMANDS_PROCESSED.labels(command=command_name, status=status)._value.get()  # type: ignore[attr-defined]


def _command_latency_count(command_name: str, status: str = "success") -> float:
    for metric in COMMAND_LATENCY.collect():
        for sample in metric.samples:
            if (
                sample.name.endswith("_count")
                and sample.labels.get("command") == command_name
...
def _message_queue_depth() -> float:
    depth_samples = next(iter(MESSAGE_QUEUE_DEPTH.collect())).samples
    return next(
        sample.value for sample in depth_samples if sample.name == "message_queue_depth"
    )


def _shard_depth(shard_label: str = "0") -> float:
    for metric in list(SHARD_QUEUE_DEPTH.collect()):
        for sample in metric.samples:
            if sample.labels.get("shard") == shard_label:
                return sample.value
    return 0.0


def test_command_router_respects_registration_order_and_case():
    """Test that the command router respects registration order and case."""
...
async def test_worker_process_matches_insensitive_trigger(
    ...
    """Test that worker process matches insensitive triggers."""
    # Arrange
    router = CommandRouter()
...
async def test_worker_process_matches_sensitive_trigger(
    ...
    """Test that worker process matches sensitive triggers."""
    # Arrange
...
async def test_worker_process_matches_regex_trigger(
    ...
    """Test that worker process matches regex triggers."""
...
async def test_worker_pool_starts_and_stops_workers(
    ...
    """Test that the worker pool starts and stops workers."""
    # Arrange
    pool_size = 2
...
async def test_worker_pool_handles_regex_triggers(
    ...
    """Test that the worker pool handles regex triggers."""
    manager, queue = worker_pool_components
...
async def test_worker_handles_api_errors_with_specific_reason(
    ...
    """Test that worker handles API errors with specific reasons."""
    await bot.app.initialize()
    context_dependencies = bot.app.context_dependencies
    ...
    await_args = worker._send_to_dlq.await_args
...
async def test_worker_middleware_execution(
    ...
    """Test worker middleware execution."""
    manager, queue = worker_pool_components
    manager.register(mock_command)
    ...
    assert events == ["first", "second", "second"]
...
async def test_worker_pool_orders_by_recipient(
    ...
    """Test that the worker pool orders messages by recipient."""
    await bot.app.initialize()
...
async def test_worker_updates_queue_metrics(
    ...
    """Test that the worker updates queue metrics."""
    manager, queue = worker_pool_components
    ...
    command_name = "metrics_command"
    initial_count = _histogram_count()
    initial_command_count = _command_count(command_name)
    initial_latency_count = _command_latency_count(command_name)
    ...
    assert _histogram_count() >= initial_count + 1
    assert _command_count(command_name) >= initial_command_count + 1
    assert _command_latency_count(command_name) >= initial_latency_count + 1
    assert _message_queue_depth() == 0
    assert _shard_depth() == 0
